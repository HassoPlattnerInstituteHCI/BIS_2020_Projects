...
namespace SurgeonSim
{
    public class PlayerLogic2 : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (attached)
            {
                lungs.transform.position = upperHandle.HandlePosition(lungs.transform.position);
                Debug.Log(lungs.transform.position);
                Debug.Log(player.transform.position);
            }

            if (lungs.transform.position.x < -4.45f && lungs.transform.position.z < -13.11f)
            {
                speechOut.Speak("Congratulations.");
        ...
        private async void OnCollisionEnter(Collision collision)
        {
            ...
            if ("Lungs" == collision.gameObject.name && !attached)
            {
                if (scalpel.activeSelf)
                {
                    ...
                    scalpel.SetActive(false);
                    await speechOut.Speak("Move the lungs to the bottom left corner.");
                    await lowerHandle.MoveToPosition(new Vector3(-4.45f, 0f, -13.11f), 0.3f, false);
                }
                else
                {
                    ...
                    await speechOut.Speak("Make a cut to the lungs.");
